...
class SyncDrivers extends Command
{
     ...
     */
    protected $signature = 'samsara:sync-drivers 
                            {--company= : Sync only a specific company by ID}
                            {--force : Force sync even if recently synced}
                            {--include-deactivated : Include deactivated drivers in sync}';

    /**
     * The console command description.
     */
    protected $description = 'Sync drivers from Samsara API for all active companies';
    ...
    public function handle(): int
    {
        ...
        $includeDeactivated = $this->option('include-deactivated');
        ...
        foreach ($companies as $company) {
            $this->line('');
            $this->info("Syncing drivers for: {$company->name} (ID: {$company->id})");
            ...
            try {
                $result = $this->syncCompanyDrivers($company, $includeDeactivated);
                ...
                Log::error("Driver sync failed for company {$company->id}", [
    ...
    /**
     * Sync drivers for a specific company.
     */
    protected function syncCompanyDrivers(Company $company, bool $includeDeactivated = false): array
    {
        ...
        foreach ($drivers as $driverData) {
            $existingDriver = Driver::forCompany($company->id)
                ->where('samsara_id', $driverData['id'])
                ->first();
            
            $dataHash = Driver::generateDataHash($driverData);

            if (!$existingDriver) {
                Driver::syncFromSamsara($driverData, $company->id);
                $created++;
            } elseif ($existingDriver->data_hash !== $dataHash) {
                Driver::syncFromSamsara($driverData, $company->id);
                $updated++;
            } else {
                $unchanged++;
