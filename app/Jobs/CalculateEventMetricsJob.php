...
class CalculateEventMetricsJob implements ShouldQueue
{
    ...
    protected function calculateMetricsForCompany(Company $company): void
    {
        ...
        $eventsQuery = SamsaraEvent::forCompany($company->id)
    ...
    protected function calculateAverageProcessingTime(int $companyId, string $dateStart, string $dateEnd): ?int
    {
        ...
        $eventsWithDuration = SamsaraEvent::forCompany($companyId)
            ->whereBetween('occurred_at', [$dateStart, $dateEnd])
            ->whereNotNull('ai_actions')
            ->get();

        $totalDuration = 0;
        $count = 0;

        foreach ($eventsWithDuration as $event) {
            $aiActions = $event->ai_actions;
    ...
    protected function calculateAverageResponseTime(int $companyId, string $dateStart, string $dateEnd): ?int
    {
        ...
        $avgMinutes = DB::table('samsara_events as e')
            ->join('notification_results as nr', 'e.id', '=', 'nr.samsara_event_id')
            ->where('e.company_id', $companyId)
            ->whereBetween('e.occurred_at', [$dateStart, $dateEnd])
            ->where('nr.success', true)
            ->selectRaw('AVG(EXTRACT(EPOCH FROM (nr.timestamp_utc - e.occurred_at)) / 60) as avg_minutes')
            ->first();
    ...
    protected function calculateNotificationMetrics(int $companyId, string $dateStart, string $dateEnd): array
    {
        ...
        $results = NotificationResult::whereHas('event', function ($query) use ($companyId, $dateStart, $dateEnd) {
        ...
        // Count throttled from notification_execution
        $throttled = SamsaraEvent::forCompany($companyId)
            ->whereBetween('occurred_at', [$dateStart, $dateEnd])
