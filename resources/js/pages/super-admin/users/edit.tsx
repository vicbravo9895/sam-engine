...
export default function UserEdit() {
    const { user, companies, roles } = usePage<{ props: Props }>().props as unknown as Props;
    const [showPassword, setShowPassword] = useState(false);

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Super Admin', href: '/super-admin' },
        { title: 'Usuarios', href: '/super-admin/users' },
        { title: user.name, href: `/super-admin/users/${user.id}/edit` },
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        form.put(`/super-admin/users/${user.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${user.name} - Super Admin`} />
